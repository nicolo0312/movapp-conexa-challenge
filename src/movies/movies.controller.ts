...
export class MoviesController {
  ...
  @Get()
  @Auth()
  @ApiResponse({status:200, description:'List of movies create in our database', type: Movie})
  @ApiResponse({status:403, description:'Forbidden. Invalid token'})
  ...
  @Get('starWars')
  @Auth(ValidRoles.administrator)
  @ApiResponse({status:200, description:'List of movies from Swapi '})
  @ApiResponse({status:403, description:'Forbidden. Invalid token'})
  getStarWarsMovies() {
  ...
  @Auth(ValidRoles.administrator)
  @ApiResponse({status:200, description:'List of movies from MovieDB API, we must search by title of the movie that we wanted '})
  @ApiResponse({status:403, description:'Forbidden. Invalid token'})
